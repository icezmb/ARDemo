...
import com.amg.ar.ui.RenderInformation;
...
public abstract class GameView extends View {
    ...
    public void setCameraAngleInDegree(float horizontal, float vertical) {
        LogUtils.e(horizontal+"--------------------"+vertical);
    ...
    public void renderItem(final Canvas canvas, final Bitmap bitmap, final DisplayableItem item) {
        ...
        if (renderInformation.isOnScreen) {
            ...
            float px=renderInformation.mPositionX + bitmap.getWidth() / 2;
            float py=renderInformation.mPositionY + bitmap.getHeight() / 2;
            item.setLocation(px,py,bitmap.getWidth()/2,bitmap.getHeight()/2);

            mMatrix.reset();
            mMatrix.setTranslate(renderInformation.mPositionX, renderInformation.mPositionY);
            mMatrix.postRotate(renderInformation.mPositionZ,px,py);
